from numpy import zeros, array, dot, linspace, arange, prod, sum, asarray, matmul
...
def RK78(U1, t1, t2, F):
    ...
    return ERK(U1, t1, t2, F, "RK7(8)")


def RK8713M(U1, t1, t2, F):
    """
    RK8(7)13M from "High order embedded Runge-Kutta formulae", P. J. Prince and J. R. Dormand
    """    

    return ERK(U1, t1, t2, F, "RK8(7)13M")


def ERK(U1, t1, t2, F, ERK_scheme):
    """
    ...
    k = RK_stages(F, U1, t1, t2, s, a, c)

    dU_sol = zeros(Nv, dtype=complex)    
    for i in range(s):    
        dU_sol += dt*b[i]*k[i]            
    ...
    # dU_error = zeros(Nv, dtype=complex)
    # for i in range(s):        
    ...
    # U2 = U1.copy()
    # for n in range(N):
    #     k = RK_stages(F, U2, t_step[n], t_step[n+1], s, a, c)
    #     dU = zeros(Nv)
    #     for i in range(s):    
    #         dU += h*b[i]*k[i]       
...
def RK_stages(F, U1, t1, t2, s, a, c):

    dt = t2 - t1
    Nv = len(U1)

    k = zeros((s, Nv), dtype=complex)
    for i in range(s):  
        dU_stage = zeros(Nv, dtype=complex)  
        for j in range(i): # range(i) porque a[i,j] = 0 para j>i
            dU_stage += dt*a[i,j]*k[j] 

        k[i,:] = F(U1 + dU_stage, t1 + c[i]*dt)

    return k
    

def GBS(U1, t1, t2, F, Nl=4):

    def modified_midpoint_scheme(U1, t1, t2, F, Ni):
   
        h = (t2 - t1)/(2*Ni)
        t = linspace(t1, t2 + h, 2*Ni+1)
        Nv = len(U1)
        
        U = zeros((2*Ni+2, Nv), dtype=complex) # 2*Ni+2 porque se necesita hasta t_(2Ni+1)
        U[0,:] = U1
        U[1,:] = U1 + h*F(U1, t1)
    ...
    U = zeros((Nl, Nv))
    for i in range(Nl):
        U[i, :] = modified_midpoint_scheme(U1, t1, t2, F, N[i])    
...
def Butcher_tableau(method_name):
    ...
    if method_name in ["RK4(5)", "RKF45"]:        
        ...
        c = array([0.0, 1/5, 3/10, 4/5, 8/9, 1.0, 1.0])        
        a = array([ 
        ...
        b = array([77./1440, 0., 0., 1771561./6289920, 32./105, 243./2560, 16807./74880, 11./270, 0., 0.])
        bs = array([11./864, 0., 0., 1771561./6289920, 32./105, 243./2560, 16807./74880, 0., 11./270, 11./270])         
        q = 7
        p = 6
        s = 10
